...
- 🔧 **Visibility:** **Prometheus/Grafana** 기반의 관제 시스템을 직접 구축하여 장애를 조기에 감지하고 대응합니다.
...
- **[자동화] Python 기반 모니터링 알람 생성 스크립트 개발 (설정 시간 6h → 30m, 92% 단축)**
  - 반복적인 CloudWatch 설정을 코드화하여 운영 리소스를 절감하고 휴먼 에러 제로화

- **[트러블슈팅] 특정 도메인 리소스 요청 시 발생하는 간헐적 통신 실패 해결 (성공률 100% 확보)**
  - 멀티 AZ 환경에서 특정 타겟 호출 시 불규칙적으로 발생하는 패킷 드롭(Drop) 현상을 비대칭 라우팅 이슈로 규명하고, 경로 최적화를 통해 연결 안정성 확보
